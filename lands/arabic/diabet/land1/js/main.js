...
const blocks = document.querySelectorAll(".blur-wrap");
...
commentImage.addEventListener('change', function (event) {
  ...
  if (commentImage.value) {
    document.getElementById('commentFoto').innerHTML = 'تم تحميل الصورة';
  } else {
    document.getElementById('commentFoto').innerHTML = 'قم بتحميل صورتك';
  }
});
...
btn.addEventListener('click', function (event) {
  ...
  if (name.value.trim() && text.value.trim()) {
    message.innerHTML = 'شكرا لك! تم إرسال تعليقك الى الاعتدال.';
    name.value = '';
    city.value = '';
    text.value = '';
    commentImage.value = '';
    ...
    message.innerHTML = 'املأ جميع الحقول!'
  }
