...
if (document.body.classList.contains('ev-date')) {
  ...
  const reviewAvatar = document.querySelectorAll('.reviews-item-avatar');

  for (let i = 0; i < reviewAvatar.length; i++) {
    const element = reviewAvatar[i];

    element.textContent = reviewAvatar[i].nextElementSibling.firstChild.nextElementSibling.textContent.trim().slice(0,1)
  }
}());
