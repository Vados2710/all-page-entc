(function($){
    ...
    //Вчерашняя дата
    Landing.prototype.yesterday = function() {
        var yestDate = new Date(Date.now() - 24*60*60*1000);

        var dd = yestDate.getDate();
        if (dd < 10) dd = '0' + dd;

        var mm = yestDate.getMonth() + 1;
        if (mm < 10) mm = '0' + mm;

        var yy = yestDate.getFullYear() % 100;
        if (yy < 10) yy = '0' + yy;

        $('.yesterday').html(dd + '.' + mm + '.' + yy);
    };
    $(function() {
