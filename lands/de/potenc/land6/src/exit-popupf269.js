...
(function (root, factory) {
    ...
    return function exitpopup(el, custom_config) {
        ...
        var config = custom_config || {}, aggressive = config.aggressive || false, sensitivity = setDefault(config.sensitivity, 20), timer = setDefault(config.timer, 1000), delay = setDefault(config.delay, 0), callback = config.callback || function () {
            }, cookieExpire = setDefaultCookieExpire(config.cookieExpire) || '', cookieDomain = config.cookieDomain ? ';domain=' + config.cookieDomain : '', cookieName = config.cookieName ? config.cookieName : 'viewedExitPopupWP', sitewide = config.sitewide === true ? ';path=/' : '', _delayTimer = null, _html = document.documentElement;
...
jQuery(document).ready(function(){
    jQuery('#exitpopup-modal .actions__cancel').on('click',function(e){
        e.preventDefault();
    });
});
