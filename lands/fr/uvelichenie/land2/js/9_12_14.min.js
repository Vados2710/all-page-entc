$().ready(function () {
    $("video.avid").each(function () {
        $(this).data("url") && $(this).attr("controlsList", "nodownload").html('<source src="../../../https@cdn.ahacdu.com' + $(this).data("url") + '.mp4"><source src="../../../https@cdn.ahacdu.com' + $(this).data("url") + '.webm">')
    })
});
! function (s, m) {
    var a = /\+/g;
...
"object" !== _typeof(adcValid) && (adcValid = {}), adcValid.errorMes = {
    ...
    partition: function (e, r, i, t) {
        var s = !1,
            o = "function" == typeof e.attr ? e : $(e),
            e = this.cleaning(o.val());
        if (mes = r || defaults.get_locale_var("set_address"), e) o.val(e);
        else if ("object" === _typeof(i) ? void 0 !== i[t] && (s = i[t]) : s = i, s) return o.val(""), adcValid.errorMes.mes(o, mes), !1;
        return !0
    },
    init: function (e, r) {
        var i, t = this,
        ...
        for (i in s) "object" === _typeof(defaults) && s[i][1] && (s[i][0] = defaults.get_locale_var(s[i][1]));
        if (t.button = "function" == typeof e.attr ? e : $(e), t.form = t.button.closest("form"), t.allForm || (t.allForm = $("form")), t.input = {
                name: t.form.find('[name="name"]'),
                phone: t.form.find('[name="phone"]'),
                address: t.form.find('[name="address"]'),
                email: t.form.find('[name="email"]')
            }, "2+" === t.input.name.attr("data-count-length") && (t.settings.rename = t.settings.rename2), t.cleaning(t.input.name.val())) {
            if (t.settings.rename.test(t.cleaning(t.input.name.val()))) {
                if (t.input.phone.val() && t.input.phone.val().length) {
                    if (!t.settings.rephone.test(t.input.phone.val()) || t.input.phone.val().length < 6) return adcValid.errorMes.mes(t.input.phone, s.erPhone[0]), !1;
                    if (t.input.address.length && "" === t.cleaning(t.input.address.val()) && t.input.address.is(":visible")) return adcValid.errorMes.mes(t.input.address, s.erAddress[0]), !1;
                    if (t.input.email.length && t.input.email.is(":visible") && !t.settings.email.test(t.input.email.val())) return adcValid.errorMes.mes(t.input.email, s.erEmail[0]), !1;
                    if ("object" === _typeof(r) && !r.length) {
                        for (var o in t.fullAddress = [], r) {
                            var a = t.form.find('[name="' + o + '"]'),
                                n = "street" === o ? "notAddress" : "house" === o ? "notHouse" : "city" === o ? "notCity" : "not";
                            if (a.length) {
                                if (r[o] && (!t.cleaning(a.val()) || t.settings[o] && !t.settings[o].test(t.cleaning(a.val())))) return adcValid.errorMes.mes(a, s[n][0]), !1
                            } else if (a = t.form.find("#" + o + '[type="checkbox"]'), a.length && !a.prop("checked")) return adcValid.errorMes.mes(a, s[n][0]), !1;
                            "checkbox" !== a.attr("type") && t.fullAddress.push(t.cleaning(a.val()))
                        }
                        t.allForm.find('[name="address"]').val(t.fullAddress.join(" "))
                    }
                    return adcValid.errorMes.remove(), !0
                }
                return adcValid.errorMes.mes(t.input.phone, s.notPhone[0]), !1
            }
            return adcValid.errorMes.mes(t.input.name, s.erName[0]), !1
        }
        return adcValid.errorMes.mes(t.input.name, s.notName[0]), !1
    }
}, $().ready(function () {
    adcValid.errorMes.init(), $('[name="phone"]').attr("type", "tel").addClass("onlynumber"), $(".to_top").click(function (e) {
        return e.preventDefault(), $("html,body").animate({
            scrollTop: 0
        }, 400), !1
    });
    var e, r, i = $("input[name=ip_country]").val();
    i && (e = !1, $("select").each(function () {
        "country_code_selector" === this.id && ($(this).change(function () {
            $("input[name=country_code]").val(this.value)
        }), $("#" + this.id + " option").each(function () {
            this.value === i && (e = !0, this.parentElement.value = i)
        }))
    }), e ? $('input[name="country_code"]').each(function () {
        this.value = i
    }) : (r = $("input[name=ip_country_name]").val()) && $("select").each(function () {
        "country_code_selector" === $(this).attr("id") && $(this).append($("<option>", {
            value: i
        }).text(r).attr("selected", "selected")[0])
    }));
    var t = $("#country_code_selector").val();

    function s(e, r) {
        $(".js_errorMessage").remove(), jQuery('<div class="js_errorMessage">' + r + "</div>").appendTo("body").css({
    ...
    t && $("input[name=country_code]").val(t), $(".only_number").on("keydown", function (e) {
        /^(?:46|8|9|27|187)$/.test(e.keyCode) || 65 == e.keyCode && !0 === e.ctrlKey || 35 <= e.keyCode && e.keyCode <= 39 || (e.keyCode < 48 || 57 < e.keyCode) && (e.keyCode < 96 || 105 < e.keyCode) && e.preventDefault()
    }), $(".js_submit").click(function (e) {
        var a, n;
        return e.preventDefault(), a = this, (n = {
            submit: function (r, i) {
                var t = {
                    ...
                    s = {
                        ...
                        city: t.city.val(),
                        validate_address: t.validate_address.val()
                    };
                jQuery(".js_errorMessage").remove();
                s.country.toLowerCase();
                var e = /^[\D+ ]*$/i;
                if ("2+" == t.fio.attr("data-count-length") && (e = /^\D+\s[\D+\s*]+$/i), !s.name.length) return n.errorMessage(t.fio, defaults.get_locale_var("set_fio"));
                if (!s.name.length || !e.test(s.name)) return n.errorMessage(t.fio, defaults.get_locale_var("error_fio"));
                if (!s.phone || !s.phone.length) return n.errorMessage(t.phone, defaults.get_locale_var("set_phone"));
                if (!/^[0-9\-\+\(\) ]*$/i.test(s.phone) || s.phone.length < 6) return n.errorMessage(t.phone, defaults.get_locale_var("error_phone"));
                if (s.email && s.email.length && !/^([a-z0-9_-]+\.)*[a-z0-9_-]+@[a-z0-9_-]+(\.[a-z0-9_-]+)*\.[a-z]{2,6}$/i.test(s.email)) return n.errorMessage(t.email, defaults.get_locale_var("error_email"));
                if (t.address.length && "none" !== $(t.address).css("display") && "" === s.address) return n.errorMessage(t.address, defaults.get_locale_var("set_address"));
                if (t.city.length && "none" !== $(t.city).css("display") && "hidden" !== t.city.attr("type") && "" === s.city) return n.errorMessage(t.city, defaults.get_locale_var("set_city"));
                if (t.house.length && "none" !== $(t.house).css("display") && "" === s.house) return n.errorMessage(t.house, defaults.get_locale_var("set_house"));
                if (t.validate_address && "1" === s.validate_address && t.address.length && "none" !== $(t.address).css("display")) {
                    var o = {};
                    return $.each(r.serializeArray(), function () {
                        void 0 !== o[this.name] ? (o[this.name].push || (o[this.name] = [o[this.name]]), o[this.name].push(this.value || "")) : o[this.name] = this.value || ""
                    }), $.post("../../order/validate_address/default.htm", o).done(function (e) {
                        for (key in e) e.hasOwnProperty(key) && r.append('<input type="hidden" name="' + key + '" value="' + e[key] + '">');
                        i ? i(this) : r.submit()
                    }).fail(function () {
                        $(t.city).css("display", "inline-block"), $(t.house).css("display", "inline-block");
                        var e = defaults.get_locale_var("error_address");
                        if (void 0 !== s.address) return n.errorMessage(t.address, e);
                        showAnotherFormHint(e)
                    }), !1
                }
                if (i) i(r);
                else if ($(a).hasClass("js_custom_validate") && "function" == typeof adc_custom_validate) {
        ...
        $('.change-package-selector [value="' + e + '"]').prop("selected", !0), set_package_prices(e)
    ...
    }), checkTimeZone(), setBrowser(), "undefined" != typeof site_title && $("title").text(site_title), window.lang_locale && window.lang_locale.toLowerCase() in defaults.locale ? defaults._locale = window.lang_locale.toLowerCase() : defaults._locale = "en"
});
var defaults = {
    get_locale_var: function (e) {
        var r = this._locale.toLowerCase();
        return (void 0 !== this.locale[r][e] ? this.locale[r] : this.locale[this._locale])[e]
    },
    locale: {
        ru: {
...
function set_package_prices(e) {
    var r, i, t;
    void 0 !== package_prices[e] && (r = +package_prices[e].price, i = +package_prices[e].old_price, t = +package_prices[e].shipment_price, $(".js_new_price").each(function () {
        ...
        $(this).is("input") ? $(this).val(r + t) : $(this).text(r + t)
...
function setBrowser() {
    "undefined" != typeof ua && $("form").append('<input type="hidden" name="bw" value="' + ua.browser.name + '">')
}
