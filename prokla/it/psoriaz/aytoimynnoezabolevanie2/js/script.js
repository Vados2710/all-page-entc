...
$('.show-img').click(function (event) {
  var text = $(this).html() == 'Clicca per visualizzare' ? 'Nascondere' : 'Clicca per visualizzare';
  $(this).html(text);
...
let reviews = function () {
  ...
  commentFileInput.change(function (e) {
    commentFileText.html('Фото&nbsp;загружено');
  ...
  $('.reviews-form').submit(function (e) {
    ...
    e.preventDefault();
    commentFileText.html('Загрузить фото');
    commentFileText.css('color', '#000');
    commentTextArea.val('');
    commentName.val('');
    alert('Ваш комментарий добавлен и будет отображаться после одобрения модератором');
