     ...
     if (settings.progress) {
          ...
          for (var i = 0; i < choices.length; i++) {
            ...
            $option.on( 'change', function() {
              ...
              if (!($('section').hasClass('start-quizz'))) {
                ...
                $('#ssbutton').html('Rispondi a tutte le domande e ottieni uno sconto');
      ...
      function getProgress() {
        var prog = (chosen.length / itemCount) * 50 + "%",
            $submit = $('#emc-submit');
        if (settings.progress) {
          $perc.text('Per te lo sconto del ' + chosen.length * 10 + '%');
  ...
  $('.start-quizz').not('#focus-quest').hide();
      ...
      $('#quest-box section:not(#focus-quest)').css('display', 'none');
          ...
          if (!($('#quest-box').is(':empty'))) {
            ...
            $('#ssbutton').html('Vieni a prendere il tuo sconto');
          };
          $('html, body').animate({
