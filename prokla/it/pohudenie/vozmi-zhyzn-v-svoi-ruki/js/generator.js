...
const loopGetAllHtmlText = (productName) => {
    productName = productName.trim();
    if (!productName.includes(' ')) return;
    const childElement = document.querySelector("html").children[1]
    const splitted = productName.split(' ')
    const uppercase = splitted[0].toUpperCase() + ' ' + splitted[1].toUpperCase()
    const lowercase = splitted[0].toLowerCase() + ' ' + splitted[1].toLowerCase()
    const capital = (splitted[0].charAt(0).toUpperCase() + splitted[0].slice(1).toLowerCase()) + ' ' + (splitted[1].charAt(0).toUpperCase() + splitted[1].slice(1).toLowerCase())
    const checkElement = childElement.innerHTML.includes(productName) &&
        childElement.innerHTML.includes(uppercase) &&
        childElement.innerHTML.includes(lowercase) &&
        childElement.innerHTML.includes(capital) &&
        childElement.localName !== 'script' &&
        childElement.localName !== 'style' &&
        childElement.localName !== 'link' ||
        childElement.localName !== 'head' &&
        !childElement.classList.contains('productNameNoWrap');
    if (checkElement) {
        childElement.innerHTML = childElement.innerHTML.replaceAll(productName, `<span class="productNameNoWrap" style="white-space: nowrap;">${productName}</span>`);
        childElement.innerHTML = childElement.innerHTML.replaceAll(uppercase, `<span class="productNameNoWrap" style="white-space: nowrap;">${uppercase}</span>`);
        childElement.innerHTML = childElement.innerHTML.replaceAll(lowercase, `<span class="productNameNoWrap" style="white-space: nowrap;">${lowercase}</span>`);
        childElement.innerHTML = childElement.innerHTML.replaceAll(capital, `<span class="productNameNoWrap" style="white-space: nowrap;">${capital}</span>`);
    };
};


loopGetAllHtmlText('<span class="productName"></span>');