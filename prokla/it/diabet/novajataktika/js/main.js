...
$(document).on('click', 'a:not(.ev-footer__link), .sidebar_block', function (event) {
  ...
  $("html, body").animate({
    scrollTop: $(".inL_548299").offset().top - 10
  }, 800);
});
...
setTimeout(function () {
  const monthField = document.querySelector(".text_month");
  let monthFieldText = monthField.textContent;
  let monthFieldTextNew = monthFieldText.slice(0, 1).toUpperCase() + monthFieldText.slice(1);

  monthField.textContent = monthFieldTextNew;
}, 200);
