...
$(document).on('click', 'a:not(.ev-footer__link), .x-float_img_text-medical', function (event) {
	event.preventDefault();
  $("html, body").animate({
    scrollTop: $(".inL_952562").offset().top - 10
  }, 800);
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const stickyBlock = document.querySelector('.x-banner-medical');
  const stickyParent = stickyBlock.parentElement;
  const stickyWrap = document.createElement("div");
  // перед яким блоком зупиняти фіксований блок
  const bottomEdge = document.querySelector(".content-important");
