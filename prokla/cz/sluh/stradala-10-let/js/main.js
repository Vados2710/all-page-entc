...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function setPositionSticky() {
    ...
    if (stickParentTop < 0) {
      ...
      if (
        stickyBlock.getBoundingClientRect().bottom >
        bottomEdge.getBoundingClientRect().top - 20
      ) {
        stickyBlock.style.top =
          bottomEdge.getBoundingClientRect().top -
          stickyBlock.getBoundingClientRect().bottom +
          "px";
      }
    } else {
...
$("a:not('#privacy-btn, #terms-btn, #disclaimer-btn')").click(function () {
  event.preventDefault();
