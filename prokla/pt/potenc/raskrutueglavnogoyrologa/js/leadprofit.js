(function($) {
    ...
    var clickId,
        ...
        upClick = function(options, callback, form) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data) console.log(callbacks, data, url);
                    progress = false;
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.error) {
                        ...
                        options.successCallback(form, data.id_order);
                    }
                }
            });
        ...
        click_go = function(a, options) {
            ...
            if (landUrl.length > 0) document.location = landUrl + (hash ? `&hash=${hash}` : '');
                ...
                url: options.landUrl,
                data: { click_id: $(a).data('clickId'), hash:hash },
                dataType: 'jsonp',
                jsonpCallback: 'callback',
                success: function(data) {
                    if (data.error) {
    ...
    $.fn.getOptionsHash = function (url) {
        const hash = $.url(url).param('hash') || $.url(url).param('cid') || null
        return hash
    };
    $.fn.leadprofit = function(options) {
        ...
        var url = location.href,
            click_id = clickId || $.url(url).param('click_id');
        ...
        options.hash = options.id = options.isHashPrioritet && options.hash ? options.hash : $.fn.getOptionsHash(url) || options.hash;
        if (options.comebacker === false) {
            comebacker_launch = false;
        } else if (options.comebacker === true) {
            comebacker_launch = true;
        }
        if (options.clickId > 0 || options.clickId > '') {
    ...
    $.fn.leadprofitSubmit = function(options) {
        ...
        var callback = function(click_id, country, form) {
            ...
            if (typeof input.val() !== "undefined") {
                input.val(click_id).data('leadprofitOptions', options);
            } else {
                $('<input>').attr({
                    type: 'hidden',
                    name: 'click_id',
    ...
    $.fn.leadprofitPreland = function(options) {
        ...
        options.hash = options.id = options.isHashPrioritet && options.hash ? options.hash : ($.url(url).param('hash') || options.hash);
        ...
        upClick(options, function(clickId, a) {
            $.ajax({
                url: options.landUrl,
                data: { click_id: clickId, hash: options.hash  },
                dataType: 'jsonp',
                type: "POST",
                jsonpCallback: 'callback',
                success: function(data) {
