...
commentImage.addEventListener('change', function(event){
    ...
    if(commentImage.value) {
        document.getElementById('commentFoto').innerHTML = 'Foto carregada';
    } else {
        document.getElementById('commentFoto').innerHTML = 'Anexar uma foto';
    }
});
...
btn.addEventListener('click', function(event){
    ...
    if(name.value.trim() && age.value.trim() && city.value.trim() && text.value.trim()) {
        message.innerHTML = 'Obrigado! Seu comentário foi enviado para moderação.';
        name.value = '';
        age.value = '';
        city.value = '';
        text.value = '';
