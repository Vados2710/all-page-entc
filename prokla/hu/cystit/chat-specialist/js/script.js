    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 4) {
                ...
                // appGender();
                // genderNext();
                generatorQuestion(['60 évnél idősebb', '30-60 év között', '30 éves korig']);
                questionNext();
            } else if (currentMessage == 5) {
                process = false;
                generatorQuestion(['Nehezen tudok válaszolni', 'Több mint 3 alkalommal óránként', '1-2 alkalommal óránként']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['Ritkán', 'Minden nap', 'Néhányszor naponta']);
                questionNext();
            } else if (currentMessage == 9) {
                process = false;
                generatorQuestion(['Ritkán', 'Minden nap', 'Állandóan']);
                questionNext();
            } else if (currentMessage == 10) {
                process = false;
                generatorQuestion(['Ritkán', 'Gyakran']);
                questionNext();
            } else if (currentMessage == 11) {
                process = false;
                generatorQuestion(['Szintetikus ', 'Természetes']);
                questionNext();
            } else if (currentMessage == 12) {
                process = false;
                generatorQuestion(['Nem számít, a lényeg, hogy segítsen', 'Természetes']);
                questionNext();
            } else if (currentMessage == 13) {
                process = false;
                generatorQuestion(['EREDMÉNY MEGTEKINTÉSE']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function simptomCheck() {
    $('.chat-content-list').append('<div class="chat-content-buttons-question chat-content-buttons-question1">' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom1">Kellemetlen érzés az ágyékban</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom2">Akaratlan vizeletvesztés</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom3">Fájdalomérzet vizeletürítésnél</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom4">Hőemelkedés</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom5">Gyengeség és szédülés</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom6">Égető érzés a húgycsőben</span></div>' + '<div class="chat-content-buttons-question-block chat-content-buttons-question-block-next"><span class="next">TOVÁBB</span></div></div>');
