...
function makeTimer(e) {
    var t = document.querySelector("#min"), o = document.querySelector("#sec"), r = setInterval((function () {
...
})), document.addEventListener("DOMContentLoaded", (() = > {moment.locale(document.getElementsByTagName("html")[0].getAttribute("lang"));
document.querySelectorAll(".js-current-date").forEach((e = > {e.innerHTML = moment().format("DD.MM.YYYY")
})),
document.querySelectorAll(".js-current-month").forEach((e = > {e.innerHTML = moment().format("MMMM")
})),
document.querySelectorAll(".js-current-year").forEach((e = > {e.innerHTML = moment().format("YYYY")
})),
document.querySelectorAll(".js-publish-date-notime").forEach((e = > {e.innerHTML = moment().add(-6, "days").format("LL")
})),
document.querySelectorAll("span[class^=day-]").forEach((e = > {let t = e.getAttribute("class").split("-")[1];
e.innerText = moment().add(-t, "days").format("DD.MM.YYYY")
}))
}))
;var resultWrapper = document.querySelector(".spin-result-wrapper-roulette"),
    wheel = document.querySelector(".wheel-img-roulette"), closePopup = document.querySelector(".close-popup-roulette");
...
class Comment {
    ...
    uploadFile(e) {
        ...
        if (e.size > 1048576) alert("El archivo debe tener menos de 1 MB."); else {
    ...
    formImageChange() {
        this.formImage.addEventListener("change", (() = > {this.uploadFile(this.formImage.files[0])
    }))
    }
    ...
    pushComBlock() {
        this.commentForm.addEventListener("submit", (e = > {if(e.preventDefault(), this.inputCommentName)
        {
    ...
    domOnloader() {
        document.addEventListener("DOMContentLoaded", (() = > {localStorage.commArr && (this.commArrAll = JSON.parse(localStorage.getItem("commArr")), commentPushBlock.innerHTML = this.commArrAll.join(""))
    }))
    }
}