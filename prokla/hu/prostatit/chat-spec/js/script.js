    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 0) {
                ...
                process = false;
                generatorQuestion(['30 éves korig', '30-50 év között', '50 évnél idősebb']);
                questionNext();
                ...
                simptomCheck(['Légszomj', 'Libidó csökkenése', 'Mellkasi fájdalmak', 'Gyors magömlés', 'Hangulatingadozás', 'Nemi aktus időtartamának csökkenése']);
                simptomNext();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Peter Muller üzenetet ír<span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function simptomCheck(arr) {
    ...
    inner += '<div class="chat-content-buttons-question-block chat-content-buttons-question-block-next"><span class="next">Válassza a lehetőséget</span></div></div>';
...
function appStart() {
    $('.chat-content-list').append('<button id="start" class="btn__start">Specialista keresése</button>');
    scrollDown();
}
