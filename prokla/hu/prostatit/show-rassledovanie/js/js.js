...
// })
let btnReslut = document.querySelector('.btn-result');
...
//         $("html, body").animate({scrollTop: $('#form').offset().top - 90}, "slow");
//         sessionStorage.clear()
//     }
// }
let resultValue = 0;
btnReslut.addEventListener('click', function(e) {
    e.preventDefault();
    let inputs = document.querySelectorAll('.question__answer:checked')
    if (inputs.length < 5) {
        alert('Válaszoljon minden kérdésre')
    } else {
        for(let i = 0; i < inputs.length; i++) {
            resultValue += +inputs[i].value;
        }
        showResult ()
    }
...
$('.popup-result, .btn-scroll, .close-popup').on('click', function () {
...
$(document).ready(function () {
    $("#root a").attr('data-scroll', '#form');
    $(function () {
        $('#root a').click(function (evt) {
            evt.preventDefault();
