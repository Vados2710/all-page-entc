...
var messages = [{
    ...
    message: 'Все указывает на то, что у вас простатит. Но не стоит впадать в отчаяние. Есть средство, которое может вам помочь — '+productName+'. Это натуральные капсулы на основе крапивы, ромашки, клюквы и сосны, которые способны восстановить мужское здоровье. '
}, {
    message: ''+productName+' помогает: — устранить боли в тазу и промежности;\n' + '— нормализовать позывы к мочеиспусканию;\n' + '— улучшить качество сексуальной жизни.\n'
}, {
    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 3) {
                ...
                process = false;
                generatorQuestion(['30 éves korig', '30-50 éves kor között', '50 éves kor felett ']);
                questionNext();
            } else if (currentMessage == 5) {
                process = false;
                generatorQuestion(['Egyszer sem', 'Egyszer egy nap', 'Talán a vizelések fele', 'Majdnem mindig ']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['Ajánlás elolvasása ']);
                questionNext();
            } else if (currentMessage == 16) {
                process = false;
                generatorQuestion(['Mennyire hatásos ez? ']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Karl Bauer üzenetet ír <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function appStart() {
    $('.chat-content-list').append('<button id="start" class="btn__start">Válaszolni a kérdésekre </button>');
    scrollDown();
}
function startNext() {
