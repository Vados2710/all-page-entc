...
function refresh() {
  ...
  const time = (min <= 9 ? "0" + min : min) + ":" + sec;

  if (document.getElementById('timer')) {
    document.getElementById('timer').innerHTML = time;
  }

  const inter = setTimeout("refresh()", 1000);
...
for (var i = 0; i < linkNav.length; i++) {
  linkNav[i].addEventListener('click', function (e) { //по клику на ссылку
    ...
    t = document.querySelector(hash).getBoundingClientRect().top - 60,  // отступ от окна браузера до id
        start = null;
