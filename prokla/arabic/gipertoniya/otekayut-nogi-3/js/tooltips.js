$(document).ready(function () {
    let tooltipColor = "rgb(61, 132, 255)";
    let tooltipPosition = "left";
    let tooltipName = "بركات مجيد";
    let tooltipPhone = "+964 XXXXXXXXXX";
    let tooltipStyle = '<style>.input-group{position:relative;}.input-group input{width:100%;box-sizing:border-box;}.input-group:before{z-index:99;content:attr(data-content)"";position:absolute;top:-10px;' + tooltipPosition + ":0;background:" + tooltipColor + ";color:#fff;border-radius:5px;padding:3px 5px;font-family: Arial;font-size: 14px}.tooltip-hide.input-group:before{display: none;}</style>";
    $(document).ready(function () {
    ...
    if (tooltipPosition == "left") {
        let tooltipStyleRtl = "<style>.input-group_phone{direction: ltr!important; text-align: left;}.input-group_phone:before{left: initial !important;right: 0;}</style>";
        $('input[name="phone"]').parent().addClass("input-group_phone");
        $(document.body).append(tooltipStyleRtl)
    }
});