...
window.addEventListener('scroll', function() {
   ...
   if (window.pageYOffset < 5800  && window.pageYOffset > 250) {
      block.style.position = 'fixed';
      block.style.top = 20 + 'px';
      block.style.bottom = 'auto';
   } else if (window.pageYOffset > 5800 && window.pageYOffset < 250) {
...
$('.try').on('click', function () {
    if ((!$(this).parent('.boxes_item').hasClass('afterlose-boxes_item')) && (!$('#boxesContainer').hasClass('stop_trying'))) {
        if (tries == 0) {
            ...
            $('.box-tooltip-title').html('<b>Du hast nichts gewonnen</b><br> Du bist gegangen <span style="color: red;">1 Versuch</span>! Versuchen Sie es nochmal!');
            $('.box-tooltip-btn').html('Versuchen');
            setTimeout(function () {
        ...
        if (tries == 1) {
            ...
            $('.box-tooltip-title').html('<b>Herzliche Glückwünsche!</b><br>Du hast einen vergünstigten <br>Uromexil Forte<b style="color: red;"> gewonnen!</b>');
            $('.box-tooltip-btn').html('Holen Sie sich Uromexil Forte mit einem Rabatt');
