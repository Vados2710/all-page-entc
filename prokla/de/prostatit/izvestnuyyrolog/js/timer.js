...
let btn = document.querySelectorAll('.spoiler-trigger');
let block = document.querySelectorAll('.spoiler-block');

function changeContent (arr, value) {
	arr.forEach((elem) => {
		if (elem.dataset.tabsHandler === value) {
...
btn.forEach(button => {
	button.addEventListener('click', () => {
		let dataValue = button.dataset.tabsField;

		changeContent(block, dataValue);
	})
})