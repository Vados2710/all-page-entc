	...
	if (process) {
		...
		if (currentMessage !== messages.length) {
			if (currentMessage == 0) {
				...
				process = false;
				generatorQuestion(["Umfrage starten"]);
				questionNext();
			} else if (currentMessage == 2) {
				process = false;
				generatorQuestion(["Männlich", "Weiblich"]);
				questionNext();
				...
				process = false;
				generatorQuestion([
					"Ich habe ein paar Kilogramme mehr",
					"Ich habe einen dünnen Körperbau",
				...
				process = false;
				generatorQuestion([
					"Ich versuche nur gesundes Essen zu essen",
					"Ich halte meine Diät nicht",
					"Manchmal kann ich es mir ausnahmsweise leisten",
				]);
				questionNext();
			} else if (currentMessage == 6) {
				process = false;
				generatorQuestion([
					"Ich führe einen inaktiven Lebensstil",
					"Ich mache regelmäßig Sport",
					"Manchmal mache ich Übungen",
				]);
				questionNext();
			} else if (currentMessage == 7) {
				process = false;
				generatorQuestion(["Umfrage fortsetzen"]);
				questionNext();
			} else if (currentMessage == 8) {
				process = false;
				generatorQuestion(["Ja", "Nein", "Ich weiss nicht"]);
				questionNext();
				...
				process = false;
				generatorQuestion(["Sehr oft", "Extrem selten", "Regelmäßig"], true);
				questionChangeNext();
			} else if (currentMessage == 13) {
				process = false;
				generatorQuestion(
					[
						"Ich gehe regelmäßig nachts auf die Toilette",
						"Ich schlafe die ganze Nacht tief und fest",
						"Manchmal verspüre ich den Harndrang, auf die Toilette zu gehen",
					],
					true
				);
				questionChangeNext();
				...
				generatorQuestion([
					...
					"Auf einige Merkmale verschlechtert",
				...
				process = false;
				generatorQuestion([
					"Manchmal, wenn ich lange nicht gegessen habe",
				...
				process = false;
				generatorQuestion(["Selten", "Oft", "Immer"]);
				questionNext();
				...
				simptomCheck([
					...
					"Ständige Apathie, Energiemangel",
				]);
				simptomNext();
				...
				generatorQuestion(
					[
						"Anfälle von anstrengendem Hunger, häufigem Wasserlassen, Kopfschmerzen",
						"Ich fühle mich gesund, es gibt keine Beschwerden",
						"Manchmal habe ich Kopfschmerzen, ich fühle allgemeine Schwäche",
					],
					true
				);
				questionChangeNext();
...
function simptomCheck(arr) {
	...
	inner +=
		'<div class="chat-content-buttons-question-block chat-content-buttons-question-block-next"><span class="next">Selecione</span></div></div>';
...
function appStart() {
	$(".chat-content-list").append(
		'<button id="start" class="btn__start">Finden Sie einen Berater</button>'
	);
	scrollDown();
}
