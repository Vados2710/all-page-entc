    ...
    if($(".mt_slider").length>0 || $(".stage_slider").length>0 || $(".ndrgallery").length>0) {
        ...
        yepnope({
            ...
            complete : function() {
                ...
                /* Provisorium: die Thumbnails werden nach dem Init der Bildergalerie nicht durch die korrekte Bildvariante ersetzt. Deshalb hier einmal das Resize-Event auslösen, um responsiveImages() auszulösen */
...
function updateSharebox(u) {
    ...
    if(navigator.userAgent.match(/(iPhone)/g) || navigator.userAgent.match(/Android\s([4-9\.]*)/i)) {
        $(".to_google").addClass("to_whatsapp").attr("href", "WhatsApp://send?text="+shareUrl).attr("title","mit WhatsApp versenden").attr("aria-label","mit WhatsApp versenden").find(".icon").removeClass("icon_google").addClass("icon_whatsapp");
        $(".to_google").find(".longtext").text("WhatsApp");
    } else {
        ...
        $(".to_google").find(".longtext").text("Google +");
    }
}
...
function showcopyrights() {
    ...
    $("img").not("[src*='/resources/images/logos/'], [src*='/images/maps/'], [src*='virtualearth.net'], [src*='ivwbox'], [src*='clublogos.png'], [src*='rueckseite.jpg'], .mfp-image, .cp, .switchbox-switches img").each(function(){
        ...
        if($(this).closest(".nocopyright").length==0) {
            ...
            /* Develop: Bildrechte aus data-Attributen auslesen und ausspielen. HELPDESK-570 */

            if(document.location.hostname.match(/172.29.139/)) {

                var title = typeof $(this).attr("title") != "undefined" ? $(this).attr("title") : "";
                var photographer = typeof $(this).data("photographer") != "undefined" ? $(this).data("photographer") : "";
                var source = typeof $(this).data("credit") != "undefined" ? $(this).data("credit") : "";
                var copyright = typeof $(this).data("copyright") != "undefined" ? $(this).data("copyright") : "";
                var licence = typeof $(this).data("licence") != "undefined" ? $(this).data("licence") : "";

                var metadata = $('<div class="imagelicence"></div>')
                    .append('<div class="imagelicence-row group"><span class="imagelicence-label">Titel:</span><span class="imagelicence-content">' + title + '</span></div>')
                    .append('<div class="imagelicence-row group"><span class="imagelicence-label">Fotograf:</span><span class="imagelicence-content">' + photographer + '</span></div>')
                    .append('<div class="imagelicence-row group"><span class="imagelicence-label">Quelle:</span><span class="imagelicence-content">' + source + '</span></div>')
                    .append('<div class="imagelicence-row group"><span class="imagelicence-label">Copyright:</span><span class="imagelicence-content">' + copyright + '</span></div>')

                if (licence != "") {
                    metadata.append('<span class="imagelicence-label">Lizenz:</span><span class="imagelicence-content"><a href="http://creativecommons.org" target="_blank">' + licence + '</a></span>')
                }
...
function anchorlinks(){
    ...
    $(".copytext h3").each(function( index ) {
...
function hotornot(i) {
    ...
    choice_one
        ...
        .find("img").attr("src", choices[0]["image"]).end()
        .find(".textpadding").html(choices[0]["caption"])


    choice_two
        .data("uuid", choices[1]["uuid"])
        .find("img").attr("src", choices[1]["image"]).end()
    ...
    instance.find(".honchoice").unbind().on("click", function(e) {
        ...
        setTimeout(function() { $(".ripple").addClass("active"); },12);
        ...
        if(choices.length - counter == 0) {
            ...
            if(instance.hasClass("landscape")) { var imgvariant = "landscape"; }
            else if(instance.hasClass("square")) { var imgvariant = "square"; }
            else if(instance.hasClass("portrait")) { var imgvariant = "portrait"; }
        ...
        setTimeout(function() {
            instance.find(".honchoice-looser")
                ...
                .find("img").attr("src", choices[counter]["image"]).end()
                .find(".textpadding").html(choices[counter]["caption"]).end()
                .removeClass("honchoice-looser");
            counter++;
            instance.find(".honcounter .count").text(choices.length - counter +1);
            if(choices.length - counter +1 == 1) { instance.find(".pluralending").hide(); }
        },500);
    });
}
...
;    $(document).ready(function() {
    var cccNow = new Date();
    var cccStart = new Date(2015, 11, 11, 5, 20, 0); /* 2014, 11, 12, 5, 20, 0 */
    var cccEnd = new Date(2015, 11, 12, 1, 0, 0);    /* 2014, 11, 13, 1, 0, 0 */
    var boxonlyStart = new Date(2015, 11, 12, 0, 0, 0); /* 2014, 11, 13, 0, 0, 0 */
    var boxonlyEnd = new Date(2015, 11, 14, 12, 0, 0);  /* 2014, 11, 15, 12, 0, 0 */
    var cccBody = $('body');
    if (!cccBody.hasClass('webradio_popup') && !cccBody.hasClass('popup') && $(".livecenter").length == 0 && cccNow > cccStart && cccNow < cccEnd) {

        $("head").append('<link href="/resources/css/charitycrawl.css" media="screen" rel="stylesheet" type="text/css">');
        $().handinhandcrawl();

    } else if ($(".hihbox").length > 0 && cccNow > boxonlyStart && cccNow < boxonlyEnd) {

        /* Kein Crawl, nur Spendenstand in der Box anzeigen */
        $.get("/public/spendenticker/spendenticker.json", function(data) {
            remoteData = eval(data);
            $(".donationamount").text(remoteData.totalAmount);
            $(".donationamountbox").removeClass("invisible");
        });
    }
});
...
;(function($) {
    $.fn.handinhandcrawl = function() {
        var ccr_busy = false;
        var defaults = {
            file : "/public/spendenticker/spendenticker.json"
        };

        var ccr_html = '<div id="ccr">';
        ccr_html += '	<div id="ccr_logo" class="ccr_logo_large"><a href="/hand_in_hand_fuer_norddeutschland/" title="Zur Infoseite: Hand in Hand f&uuml;r Norddeutschland"></a></div>';
        ccr_html += '	<div id="ccr_total"><span class="nb40">GESAMT</span>SUMME: <span id="sum">0</span></div>';
        ccr_html += '	<div id="ccr_crawl"><div id="ccr_inner" class="scrollingtext"></div></div>';
        ccr_html += '	<div id="ccr_close"><span class="icon icon_close"></span></div>';
        ccr_html += '</div>';

        $("body").append(ccr_html);

        $("#ccr_close").bind("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            ccr_stop();
        });

        $(".ccr_logo_mini").live("click", function(e) {
            e.preventDefault();
            e.stopPropagation();
            ccr_reappear();
        });

        if($.cookie("ndrcharity") == "no_ccr") {
            ccr_stop();

            // Auch bei gesetztem Cookie soll ggf. die Spendensumme aktualisiert werden.
            if($(".donationamountbox").hasClass("invisible")) {
                $.get("/public/spendenticker/spendenticker.json", function(data) {
                    remoteData = eval(data);
                    $(".donationamount").text(remoteData.totalAmount);
                    $(".donationamountbox").removeClass("invisible");
        ...
        function step() {
            $("#ccr_inner ul").animate({
                "left": "-=100px"
            },2000,"linear",function() {
                if (parseInt($("#ccr_inner ul:first").css("left")) < -$("#ccr_inner ul:first").width()) {
                    $("#ccr_inner ul:first").remove();
                }

                if($("#ccr_inner li:last").offset().left < $("body").width()) {
                    if (ccr_busy === false) {
                        ccr_busy = true;
                        getData();
                    }
        ...
        function getData() {
            var d = new Date();
            var s = d.getTime();
            $.get(defaults.file+"?c="+s, function(data) {
                remoteData = eval(data);
                $("#ccr_total span#sum, .donationamount").text(remoteData.totalAmount);

                /* Gesamtsumme in der Box anzeigen */
                $(".donationamountbox").removeClass("invisible");

                var item = "<ul>";
                $(remoteData.latestDonations).each(function(e,i){
                    item += "<li>";
                    item +=	remoteData.latestDonations[e].donator;
                    if (remoteData.latestDonations[e].donatorLocation) {
                        item +=	", "+remoteData.latestDonations[e].donatorLocation;
                    }
                    item +=	": " +remoteData.latestDonations[e].amount;
                    item += "</li>";
                });
                item += "</ul>";
                if ($("#ccr_inner li").length != 0) {
                    var pos = 20 + $("#ccr_inner li:last").offset().left + $("#ccr_inner li:last").width();
                }
                $(item).css({"left":pos,"white-space":"nowrap"}).appendTo("#ccr_inner");
                ccr_busy = false;
                step();
            });
        }
        ...
        function ccr_stop() {
            $("#ccr_inner ul").stop();
            $("#ccr").addClass("off");
            $("#ccr_logo").removeClass("ccr_logo_large").addClass("ccr_logo_mini");
            $.cookie('ndrcharity','no_ccr',{expires: 1, path: '/'});
        }

        function ccr_reappear() {
            $("#ccr").removeClass("off");
            window.setTimeout(function() {$("#ccr_logo").removeClass("ccr_logo_mini").addClass("ccr_logo_large");},250) ;

            $.cookie('ndrcharity','null',{expires: 1, path: '/'});
            getData();
        }
...
;$(document).ready(function() {
    var socialbar = $('<div id="staticsocialbar" class="off"><div class="staticsocialbar-inner group"><a href="#" class="button socialmedia to_facebook"><span class="icon icon_facebook"></span><span class="longtext">Facebook</span></a><a href="#" class="button socialmedia to_twitter"><span class="icon icon_twitter"></span><span class="longtext">Twitter</span></a><a href="#" class="button socialmedia to_google"><span class="icon icon_google"></span><span class="longtext">Google +</span></a><a href="#top" class="button to_top"><span class="icon icon_arrow_up"></span><span class="longtext">Seitenanfang</span></a></div></div>');
...
$(document).scroll(function() {
    ...
    if(s > 160) {
        $("#staticsocialbar").removeClass("off");
    } else {
        $("#staticsocialbar").addClass("off");
    }
});
    ...
    (function() {
        var formControl = {
            ...
            numberOfPages: function() {
                return $('.js-form-wrapper > [class*="page-"]').size();
            },
