...
$(document).ready(function () {
	if ($(window).width() > 1152) {
		//check if window has less px than 800, because share-button is being hidden by css
		if ($(this).scrollTop() >= $('.read-more').position().top + $('.read-more').height()) {
	...
	$(document).on('scroll', function () {
		if ($(window).width() > 1152 && $(window).width()) {
			//check if window has less px than 800, because share-button is being hidden by css
	...
	if (commentstring != undefined) {
		...
		$.each(commentArr, function (index, value) {
			if (value) {
				...
				tempComment =
					'<hr class="commentunderline"><div class="profile"><a class="profileimage" href="" target="_blank"><img src="img/user.jpg" alt="profilowe"></a><div class="commentstextcontainer"><a class="commentname" href="" target="_blank">' +
					obj.fullname +
					'</a><div class="commentcontent"><div class="commenttext">' +
					obj.text +
					'</div><div class="commentactions"><span class="commenttime">' +
					finalDateStr +
					' temu</span></div></div></div></div>';
