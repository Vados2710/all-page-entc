    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 3) {
                ...
                process = false;
                generatorQuestion(['Nigdy', 'Raz dziennie', 'W około połowie przypadków oddawania moczu', 'Prawie zawsze']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['Przeczytaj zalecenie']);
                questionNext();
            } else if (currentMessage == 16) {
                process = false;
                generatorQuestion(['Jak skuteczny jest ten środek?']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Karol Brzeziński pisze wiadomość... <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function appStart() {
    $('.chat-content-list').append('<button id="start" class="btn__start">Odpowiedz na pytania</button>');
    scrollDown();
}
function startNext() {
