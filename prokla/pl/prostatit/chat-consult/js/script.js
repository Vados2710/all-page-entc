    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 0) {
                ...
                process = false;
                generatorQuestion(['poniżej 30 lat ', 'od 30 do 50 lat', 'Powyżej 50 lat']);
                questionNext();
            } else if (currentMessage == 5) {
                process = false;
                generatorQuestion(['Tak', 'Nie']);
                questionNext();
                ...
                simptomCheck(['Duszność', 'Zmniejszone libido', 'Ból w okolicy serca', 'Szybki wytrysk', 'Wahania nastroju', 'Skrócenie czasu trwania stosunku']);
                simptomNext();
                ...
                process = false;
                generatorQuestion(['Uzyskaj wyniki']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Piotr Marcinowski pisze wiadomość<span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function simptomCheck(arr) {
    ...
    inner += '<div class="chat-content-buttons-question-block chat-content-buttons-question-block-next"><span class="next">Wybierz</span></div></div>';
...
function appStart() {
    $('.chat-content-list').append('<button id="start" class="btn__start">Znajdź specjalistę</button>');
    scrollDown();
}
