...
function add() {
	...
	if ((commentText.length < 5) || (name.length < 3) || (name.length < 3)) {
		...
		if (Cookies.get("commentArr") != undefined) {
			...
			Cookies.set('commentArr', commentArr + ',/,', {
				expires: 1
			});
		} else {
			let comment = new Comment(fullname, commentText, new Date());
...
function hide_calc(){
	let calc = document.getElementsByClassName("kalkulator")[0];
	calc.classList.add("hide");
	let calc1 = document.getElementsByClassName("info_kalk")[0];
	calc1.classList.add("show");
	document.getElementsByClassName("info_kalk")[0].style.display= 'block';
