...
commentImage.addEventListener('change', function(event){
    ...
    if(commentImage.value) {
        document.getElementById('commentFoto').innerHTML = 'Nuotrauka įkelta';
    } else {
        document.getElementById('commentFoto').innerHTML = 'Pridėkite nuotrauką';
    }
});
...
btn.addEventListener('click', function(event){
    ...
    if(name.value.trim() && city.value.trim() && text.value.trim()) {
        message.innerHTML = 'Dėkoju! Jūsų apžvalga išsiųsta moderuoti.';
        name.value = '';
        city.value = '';
        text.value = '';
        commentImage.value = '';
