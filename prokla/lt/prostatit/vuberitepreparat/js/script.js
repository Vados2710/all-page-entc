  ...
  if (process) {
    if (currentMessage !== messages.length) {
      if (currentMessage == 3) {
        ...
        process = false;
        generatorQuestion(['iki 30 metų', 'nuo 30 iki 50 metų', 'virš 50 metų']);
        questionNext();
      } else if (currentMessage == 5) {
        process = false;
        generatorQuestion(['niekada', 'vieną kartą į parą', 'apytiksliai pusę šlapinimosi kartų', 'beveik visada']);
        questionNext();
        ...
        process = false;
        generatorQuestion(['Skaityti rekomendacijas']);
        questionNext();
      } else if (currentMessage == 16) {
        process = false;
        generatorQuestion(['Kiek tai efektyvu?']);
        questionNext();
      } else {
        process = false;
        showMessage();
...
function addMessage() {
  var body_mas = '<div id="promo" class="writing"><p>Laimonas Sukutis renka tekstą <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function appStart() {
  $('.chat-content-list').append('<button id="start" class="btn__start">Atsakyti į klausimus</button>');
  scrollDown();
}
