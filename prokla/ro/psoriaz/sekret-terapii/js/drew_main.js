	...
	if (sendContainer) {
		...
		send.addEventListener("click", function(event) {
			if (!textarea.value) {
				...
				if (commentNameInput) {
					commentInput.insertAdjacentHTML(
						"afterEnd",
						'<div class="comments-item"><div class="comment-avatar"><img height="48" src="img/ava.jpg" width="48"></div><div class="comment-text"> <span class="comment-username"><a href="{landing_url}">' +
							commentNameInput.value +
							"</a></span>" +
							textarea.value +
							'</div><div class="comment-action"><span class="like">Ca</span><span class="reply"><a href="{landing_url}">Răspuns</a></span><span class="like-count">0</span><span class="comment-date">tocmai acum</span>'
					);
					textarea.value = "";
					commentNameInput.value = "";
					likeCount();
		...
		var commentsRefreshing = document.querySelector(
			".comments-refreshing-wrapper"
		);
		if (
			commentsRefreshing &&
			commentsRefreshing.querySelector(".comments-item")
		) {
			var commentFlag = 0;
			commentsRefreshing
				.querySelector(".comments-item")
				.classList.add("comments-newly-showed");
			var commentsRefreshingCords =
				commentsRefreshing.getBoundingClientRect().top + pageYOffset;
			console.log(commentsRefreshing.getBoundingClientRect().top + pageYOffset);
			window.addEventListener("scroll", function() {
				console.log(
					"Element Total Position: " +
						commentsRefreshingCords +
						"; Page scroll: " +
						pageYOffset +
						"; Element Window Position: " +
						commentsRefreshing.getBoundingClientRect().top
				);
				if (
					commentsRefreshing.getBoundingClientRect().top <= 500 &&
					commentFlag == 0
				) {
					++commentFlag;
					var counter = 4000;
					setTimeout(
						commentRefresh,
						counter,
						commentsRefreshing.querySelectorAll(".comments-item"),
						1
					);
				}
			});
