...
function launchFrom(t) {
  if (rockets.length < 10) {
    var e = new Rocket(t);
    (e.explosionColor = 10 * Math.floor((360 * Math.random()) / 10)),
      (e.vel.y = -3 * Math.random() - 4),
      (e.vel.x = 6 * Math.random() - 3),
      (e.size = 8),
      (e.shrink = 0.999),
      (e.gravity = 0.01),
      rockets.push(e);
...
function loop() {
  ...
  for (var t = [], e = 0; e < rockets.length; e++) {
    ...
    var o = Math.sqrt(
        Math.pow(mousePos.x - rockets[e].pos.x, 2) +
          Math.pow(mousePos.y - rockets[e].pos.y, 2)
      ),
      s =
        rockets[e].pos.y < (2 * SCREEN_HEIGHT) / 3 && 100 * Math.random() <= 1;
    rockets[e].pos.y < SCREEN_HEIGHT / 5 || rockets[e].vel.y >= 0 || o < 50 || s
      ? rockets[e].explode()
      : t.push(rockets[e]);
  }
  rockets = t;
  ...
  for (e = 0; e < particles.length; e++)
    particles[e].update(),
      particles[e].exists() &&
        (particles[e].render(context), r.push(particles[e]));
  for (particles = r; particles.length > MAX_PARTICLES; ) particles.shift();
}
function Particle(t) {
  (this.pos = { x: t ? t.x : 0, y: t ? t.y : 0 }),
    (this.vel = { x: 0, y: 0 }),
    (this.shrink = 0.97),
    (this.size = 2),
    (this.resistance = 1),
  ...
  (Particle.prototype.render = function (t) {
    if (this.exists()) {
      ...
      var e = this.pos.x,
        o = this.pos.y,
        s = this.size / 2,
        r = t.createRadialGradient(e, o, 0.1, e, o, s);
      r.addColorStop(0.1, "rgba(255,255,255," + this.alpha + ")"),
        r.addColorStop(
          0.8,
          "hsla(" + this.color + ", 100%, 50%, " + this.alpha + ")"
        ),
        r.addColorStop(1, "hsla(" + this.color + ", 100%, 50%, 0.1)"),
        (t.fillStyle = r),
        t.beginPath(),
        t.arc(
          this.pos.x,
  ...
  (Rocket.prototype.explode = function () {
    for (var t = 10 * Math.random() + 80, e = 0; e < t; e++) {
      var o = new Particle(this.pos),
        ...
        r = 15 * Math.cos((Math.random() * Math.PI) / 2);
      (o.vel.x = Math.cos(s) * r),
        (o.vel.y = Math.sin(s) * r),
        (o.size = 10),
        (o.gravity = 0.2),
        (o.resistance = 0.92),
        (o.shrink = 0.05 * Math.random() + 0.93),
        (o.flick = !0),
        (o.color = this.explosionColor),
        particles.push(o);
    }
  }),
  (Rocket.prototype.render = function (t) {
    if (this.exists()) {
      ...
      var e = this.pos.x,
        ...
        r = t.createRadialGradient(e, o, 0.1, e, o, s);
      r.addColorStop(0.1, "rgba(255, 255, 255 ," + this.alpha + ")"),
        r.addColorStop(1, "rgba(0, 0, 0, " + this.alpha + ")"),
        (t.fillStyle = r),
        t.beginPath(),
        t.arc(
          this.pos.x,
