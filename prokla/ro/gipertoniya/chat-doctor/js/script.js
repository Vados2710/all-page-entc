...
var messages = [{
    ...
    message: 'Последней уникальной разработкой специалистов в области сердечно-сосудистых заболеваний, показавшей наилучшие результаты, является средство Friocard.'
}, {
    message: '<picture><source media="(max-width: 600px)" srcset="' + path + 'images/tov_mob.webp" type="image/webp"><source media="(max-width: 600px)" srcset="' + path + 'images/tov_mob.png"><source srcset="' + path + 'images/tov.webp" type="image/webp"><img src="' + path + 'images/tov.png">'
}, {
    message: 'Friocard – специальные капсулы, которые помогают регулировать работу сердечно-сосудистой системы. В составе продукта три уникальных природных компонента, работающих в синергии друг с другом: боярышник однопестичный, мелисса лекарственная и экстракт чеснока. '
}, {
    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 4) {
                ...
                // appGender();
                // genderNext();
                generatorQuestion(['ub 30 de ani', ' Între 30 şi 50 de ani', 'Peste 50 de ani']);
                questionNext();
            } else if (currentMessage == 5) {
                process = false;
                generatorQuestion(['Rar', '2-4 ori pe săptămână', ' 2-4 ori pe lună']);
                questionNext();
                ...
                process = false;
                generatorQuestion([' 2-4 ori pe lună ', '2-4 ori pe săptămână', 'Nu măsor tensiunea']);
                questionNext();
            } else if (currentMessage == 9) {
                process = false;
                generatorQuestion(['Deseori ', 'Rar', 'Nu ştiu']);
                questionNext();
            } else if (currentMessage == 10) {
                process = false;
                generatorQuestion(['Oboseală frecventă, lipsă de energie', 'Uneori simt slabiciune în tot corpul', 'Nimic critic']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['Obțineți rezultatele']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Ana Alexandrescu tastarea unui mesaj <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function simptomCheck() {
    $('.chat-content-list').append('<div class="chat-content-buttons-question chat-content-buttons-question1">' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom1">Dispnee</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom2">Zgomot în urechi</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom3">Durere în regiunea inimii </span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom4">Roșeața feței </span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom5">Transpiraţie</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom6">Amețeli</span></div>' + '<div class="chat-content-buttons-question-block chat-content-buttons-question-block-next"><span class="next">Mai departe</span></div></div>');
...
function showForm() {
    ...
    var b1 = document.querySelector('.features-wrapper ');
...
document.querySelector('.yesterday2').innerHTML = addDays(new Date(), 1).toLocaleString('ru', options);
document.querySelector('.today').innerHTML = addDays(new Date(), 0).toLocaleString('ru', options);