...
for (let i = 0; i < links.length; ++i) {
    links[i].addEventListener('click', function(event) {
        ...
        let elemID = links[i].getAttribute('href');
...
// Sidebar reviews banner
const productBanner = document.querySelector('.banner_cta'),
      reviewsCarousel = document.querySelector('.main-carousel-wrapper'),
      carouselItems = document.querySelector('.carousel-items');

window.addEventListener('scroll', function() {
    if (document.documentElement.scrollTop > productBanner.offsetTop) {
        reviewsCarousel.classList.add('visible');
    } else {
        reviewsCarousel.classList.remove('visible')
    }
});
