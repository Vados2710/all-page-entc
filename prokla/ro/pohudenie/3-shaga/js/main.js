...
commentImage.addEventListener('change', function(event){
    ...
    if(commentImage.value) {
        document.getElementById('commentFoto').innerHTML = 'Фото&nbsp;загружено';
    } else {
        document.getElementById('commentFoto').innerHTML = 'Прикрепить&nbsp;фото';
    }
});
...
btn.addEventListener('click', function(event){
    ...
    if(name.value.trim() && city.value.trim() && text.value.trim()) {
        message.innerHTML = 'Спасибо! Ваш отзыв отправлен на модерацию.';
        name.value = '';
        city.value = '';
        text.value = '';
        commentImage.value = '';
