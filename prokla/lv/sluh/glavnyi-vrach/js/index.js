...
function postDate(sa, countryName, isAbbreviated) {
  ...
  var sa = sa || "dd.mm.yyyy",
    msInDay = 86400000,
    counterLength = 400, // Максимальна кількість вімотаних днів. Змінюємо за необхідності.
    months,
...
var reviews = function () {
  var e = $(".input-file"),
    t = $(".reviews-popup"),
    n = $(".reviews-form__file-text");
  e.change(function (e) {
    n.html("Fotografia încărcatăо");
  }),
    $(".reviews-form").click(function (e) {
		if ($(".input-message").val().length >= 5) {
			$(".input-name").val() &&
			$(".input-message").val() && 
			(t.fadeIn(),
			e.preventDefault(),
  ...
  $(document).ready(function () {
    ...
    $(window).width() > 1023 &&
      ($(window).mouseout(function (t) {
        t.pageY - $(window).scrollTop() < 1 &&
          1 == e &&
          ($(".exitblock").fadeIn(300), (e = !1));
      }),
      $(".modaltext").on("click", function (e) {
        ($(e.target).is($(".modaltext")) || $(e.target).is(".closeblock")) &&
...
let clos2e = document.querySelector(".closeblock"),
  button2 = document.querySelector(".out-comebacker-content-btn");
function hidePopup() {
  $(".exitblock").fadeOut(300);
}
...
$('#push_mlyjekpdfdd1__submit').click(function(e) {
