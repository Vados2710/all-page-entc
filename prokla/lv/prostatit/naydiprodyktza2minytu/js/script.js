...
var messages = [{
  ...
  message: 'Все указывает на то, что у вас простатит. Но не стоит впадать в отчаяние. Есть средство, которое может вам помочь — Urotrin. Это натуральные капсулы на основе крапивы, ромашки, клюквы и сосны, которые способны восстановить мужское здоровье. '
}, {
  message: 'Urotrin помогает: — устранить боли в тазу и промежности;\n' + '— нормализовать позывы к мочеиспусканию;\n' + '— улучшить качество сексуальной жизни.\n'
}, {
  ...
  if (process) {
    if (currentMessage !== messages.length) {
      if (currentMessage == 3) {
        ...
        process = false;
        generatorQuestion(['Līdz 30 gadiem ', 'No 30 līdz 50 gadiem', 'Vairāk par 50 gadiem ']);
        questionNext();
      } else if (currentMessage == 5) {
        process = false;
        generatorQuestion(['Ne reizi', 'Vienu reizi diennaktī', 'Apmēram pusē urinēšanu', 'Gandrīz vienmēr']);
        questionNext();
        ...
        process = false;
        generatorQuestion(['Lasīt ieteikumu']);
        questionNext();
      } else if (currentMessage == 16) {
        process = false;
        generatorQuestion(['Cik tas ir efektīvi? ']);
        questionNext();
      } else {
        process = false;
        showMessage();
...
function addMessage() {
  var body_mas = '<div id="promo" class="writing"><p>Kārlis Bauers raksta ziņojumu <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function appStart() {
  $('.chat-content-list').append('<button id="start" class="btn__start">Atbildēt uz jautājumiem   </button>');
  scrollDown();
}
