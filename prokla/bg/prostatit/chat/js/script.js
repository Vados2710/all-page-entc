    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 3) {
                ...
                process = false;
                generatorQuestion(['Нито веднъж', 'Веднъж на денонощие', 'Приблизително при всяко второ уриниране', 'почти винаги']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['Прочетете препоръките']);
                questionNext();
            } else if (currentMessage == 16) {
                process = false;
                generatorQuestion(['Доколко ефективно е средството?']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Стоян Илиев пише съобщениe <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function appStart() {
    $('.chat-content-list').append('<button id="start" class="btn__start">Отговорете на въпроси</button>');
    scrollDown();
}
function startNext() {
