...
(function () {
  ...
  var LB = function LB(params) {
    ...
    this.params = $.extend({
      comebacker: false,
      projectDomain: this.queryGET('host') ? "//".concat(this.queryGET('host'), "default.htm") : '//lea' + 'dbit.biz/',
      formDomain: this.queryGET('form_host') ? "//".concat(this.queryGET('form_host'), "default.htm") : '//lea' + 'dbit.com/',
      now: new Date().getTime(),
      commentSelector: '',
      isParked: false,
      currentUrl: document.location.hostname + document.location.pathname,
      apiUrl: "//".concat(document.location.hostname + document.location.pathname).concat(document.location.pathname[document.location.pathname.length - 1] !== 'default.htm' && 'default.htm', "api/"),
      getToExtend: ['cb', 'fblp', 'lp', 'fbsop', 'gclid'] // send to hidden inputs form

    }, params);

    if (window.location.hash === '#devmode') {
      this.params.projectDomain = this.queryGET('host') ? "//".concat(this.queryGET('host'), "default.htm") : '../https@devt.lea' + 'dbit.biz/';
      this.params.formDomain = this.queryGET('form_host') ? "//".concat(this.queryGET('form_host'), "default.htm") : '../https@devt.lea' + 'dbit.com/';
    }
  ...
  LB.prototype = {
    ...
    getTid: function getTid(cookieParked) {
      ...
      var url = cookieParked ? "//".concat(location.hostname, "api/get-tid/default.htm").concat(hash) : "api/tid/".concat(hash);
    ...
    getTidCallback: function getTidCallback(data) {
      ...
      var formAction = "".concat(cookieParked && 'default.htm', "api/conversion/new-from-form@TID=").concat(data.tid);
      ...
      $.ajax({
        url: "".concat(cookieParked ? "//".concat(location.hostname, "default.htm") : '', "api/check-page"),
        data: data,
        contentType: 'application/json',
        jsonpCallback: 'App.jsonCallback',
        dataType: 'jsonp'
