$(function () {
  var
    ...
    date = new Date();
  $('.current-day').html(days[date.getDay()]);
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const stickyBlock = document.querySelector('.inL_64001');
  const stickyParent = stickyBlock.parentElement;
  const stickyWrap = document.createElement("div");
  // перед яким блоком зупинити фіксований блок
  const bottomEdge = document.querySelector(".container__wrapper--comments");

  stickyWrap.classList.add("sticky-wrap");
  stickyParent.append(stickyWrap);
  stickyWrap.append(stickyBlock);
