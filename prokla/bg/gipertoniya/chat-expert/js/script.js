...
var messages = [{
    ...
    message: 'Как Често у вас появляется пульсирующая боль в голове?'
}, {
    message: 'Были случаи инсульта и инфаркта в вашей семье?'
}, {
    message: 'Отметьте симптомы, которые вас беспокоят'
}, {
    message: 'Как Често у вас повышается давление?'
}, {
    message: 'Как Често у вас бывает учащенный ритм сердца?'
}, {
    ...
    if (process) {
        if (currentMessage !== messages.length) {
            if (currentMessage == 4) {
                ...
                // appGender();
                // genderNext();
                generatorQuestion(['Под 30 години', 'От 30 до 50 години', 'Над 50 години']);
                questionNext();
            } else if (currentMessage == 5) {
                process = false;
                generatorQuestion(['Рядко', '2-4 пъти на седмица', '2-4 пъти на месец']);
                questionNext();
            } else if (currentMessage == 6) {
                process = false;
                generatorQuestion(['Да', 'Не', 'Не знам']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['2-4 пъти на месец ', '2-4 пъти на седмица', 'Не измервам кръвното си налягане']);
                questionNext();
            } else if (currentMessage == 9) {
                process = false;
                generatorQuestion(['Често ', 'Рядко', 'Не знам']);
                questionNext();
            } else if (currentMessage == 10) {
                process = false;
                generatorQuestion(['Честа умора, липса на енергия ', 'Понякога усещам слабост в цялото тяло', 'Нищо сериозно']);
                questionNext();
            } else if (currentMessage == 11) {
                process = false;
                generatorQuestion(['Да ', 'Не']);
                questionNext();
                ...
                process = false;
                generatorQuestion(['КЪМ РЕЗУЛТАТИТЕ']);
                questionNext();
            } else {
                process = false;
                showMessage();
...
function addMessage() {
    var body_mas = '<div id="promo" class="writing"><p>Милена Георгиева въвеждане на съобщение <span class="dot-animate-1">.</span><span class="dot-animate-2">.</span><span class="dot-animate-3">.</span></p></div>';
...
function simptomCheck() {
    $('.chat-content-list').append('<div class="chat-content-buttons-question chat-content-buttons-question1">' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom1">Задух</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom2">Шум в ушите</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom3">Болки в сърдечната област</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom4">Зачервяване на лицето</span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom5">Изпотяване </span></div>' + '<div class="chat-content-buttons-question-block simptom"><span class="simptom6">Световъртеж </span></div>' + '<div class="chat-content-buttons-question-block chat-content-buttons-question-block-next"><span class="next">Продължете</span></div></div>');
