  ...
  var ava = document.querySelectorAll('.comment__ava');
  var url = window.cdn_path || '';

  for(let i = 0; i < ava.length; i++) {
      var src = ava[i].getAttribute('src');
      ava[i].outerHTML = '<div class="ava-wrap ava-wrap'+i+'">'+ava[i].outerHTML+'</div>';
      
      document.querySelector('.ava-wrap'+i+'').style['background-image'] = "url("+ src +")";
    //  console.log(document.querySelector('.ava-wrap'+i+''))
  }