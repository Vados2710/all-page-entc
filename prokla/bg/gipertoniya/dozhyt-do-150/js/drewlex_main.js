...
function initFeatures(settings) {
    ...
    function genLocalForm () {
        var localForms = {
            ...
            'ru' : '<div class="order_block"> <div class="prod_img_wrapper"> <img src="' + settings.form.img + '" width="300" class="prod_img">' + ((settings.form.price) ? '<div class="priceс"><span class="discountс">Povoljno!</span> <s><span class="js_old_price price_old">' + settings.priceMain * 2 + settings.currencyMain + '</span></s><br> <b class="new_price js_new_price price_main">' + settings.priceMain + settings.currencyMain + '</b> </div>' : '') + '</div> <center> <h3 style="text-align: center">Naručite s popustom dok ponuda ne bude prosleđena sledećem čitaocu! <br>Ponuda će biti nevažeća za:  <span class="time_remains" id="min"></span> : <span class="time_remains" id="sec"></span> </h3> </center> <form class="order_form" action="getform.php" method="POST"> <select class="country_select wheel_input" name="country">' + ((!!settings.formApi && settings.formApi.isNeeded) ? formApiCode : '</select>')  + ' <label for="name" class="data_label">Vaše ime:</label><input class="wheel_input" type="text" name="name" required="" id="name" placeholder="Petar Petrović "><label for="phone" class="data_label">Broj telefona:</label> <input class="wheel_input" type="text" id="phone" name="phone" required="" placeholder="+387..."> <input type="hidden" name="landing" value="' + ((!!settings.formApi && settings.formApi.isNeeded) ? settings.formApi.landingValue : window.location.host) + '"> <button class="main-link" type="submit">Naručiti Hipertonin s popustom od 50%!</button> </form> </div><center> <h2 class="time_remains_title">PONUDA VAŽI DO (ISKLJUČIVO):  <span class="time_remains">' + outputDat(0, false) + '</span></h2> <br> </center>',
  ...
  var commentsRefreshing = document.querySelector('.comments-refreshing-wrapper');
	...
	var wheelBlocks = {
        ...
        'ru': { 
            wheel: '<span class="toform"></span><div class="spin-wrapper"><p><b>Pažnja!</b><br>Posetioci naše stranice imaju ekskluzivnu mogućnost da naruče Hipertonin s popustom do 50%! Da biste to uradili, zavrtite kolo sreće tako što ćete kliknuti na taster „SPIN“ (ZAVRTI) i zatim sačekajte da se potpuno zaustavi. Ko zna, možda ste baš Vi taj srećnik koji će danas uspeti malo da uštedi! Srećno!</p> <div class="wheel-wrapper"> <div class="wheel"> <img alt="" class="wheel-img" src="' + (settings.wheel.customWheel || featuresFilesSrc.prizeWheel) + '"> <div class="wheel-cursor"> <img alt="" src="' + featuresFilesSrc.wheelCursor + '"><span class="cursor-text">SPIN</span></div> </div> </div> </div>',
			popup: '<div class="spin-result-wrapper"> <div class="pop-up-window"> <div class="close-popup"></div> <span class="pop-up-heading">Čestitamo!</span> <p class="pop-up-text">Možete da naručite Hipertonin s popustom od 50%! </p> <a class="pop-up-button" href="#">Ok</a> </div> </div>',
