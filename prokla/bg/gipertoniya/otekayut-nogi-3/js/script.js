...
//EV Script | postLocalComment v1.3 | MV

//Variables | Notifications | RU
const doneTitle = "Коментарът е публикуван!",
  doneText = "Благодаря за прегледа!",
  errorInputTitle = "Грешка при публикуване!",
  errorInputText = "Някои полета са попълнени неправилно.",
  errorSendTitle = "Вече сте оставили коментар преди.",
  errorSendText =
    "Съжаляваме за неудобството, според правилата за използване на сайта последващите коментари се модерират преди да бъдат публикувани.",
  emptyTownField = "Крия",
  avatarUploaded = "Качен аватар",
  imgUploaded = "Качено изображение",
  fileTypeError = "Грешен тип файл",
  fileSizeError = "Невалиден размер на файла (fino\u00A0a\u00A02MB)",
  fileAllError = "Невалиден тип и размер на файла (fino\u00A0a\u00A02MB)";
...
const nameInput = document.querySelector(".ev-feedback__field--name") || false,
  ...
  commentInput =
    document.querySelector(".ev-feedback__field--comment") || false,
  avatarInput =
    document.querySelector(".ev-feedback__chooser--avatar") || false,
...
modalWindow.addEventListener("click", function (e) {
  ...
  targetEl.classList.contains("ev-modal") ||
  targetEl.classList.contains("ev-modal__close") ||
  targetEl.classList.contains("ev-modal__btn")
...
function setImgToLocalStorage(key, inputFile) {
  ...
  if (isImage && isSize) {
    ...
    reader.onload = function () {
      ...
      localStorage.getItem("isPublished") === null
...
function setItemsToLocalStorage() {
  ...
  commentInput
    ? setTextToLocalStorage("commentValue", commentInput.value)
    : false;
...
function createComment() {
  ...
  answerTown !== null
    ? (answerTown.innerText =
        localStorage.getItem("townValue") !== null
          ? localStorage.getItem("townValue")
          : "")
    : false;
  answerAge !== null
    ? (answerAge.innerText =
        localStorage.getItem("ageValue") !== null
...
function changeFormStyles() {
  const feedbackMain = document.querySelector(".ev-feedback"),
    ...
    dataInputs = document.querySelectorAll(
      ".ev-feedback__profile .ev-feedback__field"
    ),
...
function changeFileInputState(
  ...
  const inputLabel = node.parentNode,
    labelText = inputLabel.children[1];
  ...
  if (typeStatus && sizeStatus) {
    ...
    !typeStatus && !sizeStatus
      ...
      : !typeStatus && sizeStatus
      ? (labelText.innerText = typeErrorText)
      : false;
  }
}
...
function hideImg(key, node) {
  localStorage.getItem(key) === null && node
