...
(function ($) {
    $(function () {
        setTimeout(function () {
            var stickTop = $(".sidebar__cont"), stickOffset = stickTop.offset();
            const stickBlock = document.querySelector(".sidebar__cont"), footer = document.querySelector(".all-form__time");
            $(window).scroll(function () {
                if ($(window).scrollTop() > stickOffset.top - 35) {
                    stickTop.css({position: "fixed", top: 10});
                    $('.sidebar__cont').addClass('active');
                } else {
                    stickTop.css({position: "relative", top: 0})
                    $('.sidebar__cont').removeClass('active');
                }
            })
        }, 500)
    })
