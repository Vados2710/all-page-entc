$(document).mouseleave(function (b) {
  if (b.clientY < 10) {
    $('.exitblock').fadeIn('fast');
  }
});
$(document).click(function (b) {
  if ($('.exitblock').is(':visible') && !$(b.target).closest('.exitblock .modaltext').length) {
    $('.exitblock').remove();
  }
});
window.onload = function () {
  var b = 0;
  var c = new Date();
  $(window).on('beforeunload', function (g) {
    var f = new Date();
    var d = Math.round((f - c) / 1000);
    push_to_binom(d);
    window.localStorage.setItem('time_spent', d);
  });
};
