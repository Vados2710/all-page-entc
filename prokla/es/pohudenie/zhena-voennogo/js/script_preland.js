...
function runLandScriptsParams(land_params) {
    ...
    window.addEventListener('load', function() {
        for ( var i=0; i < document.links.length; i++ ){
            document.links[i].setAttribute('href', land_params['landing_url']);
        }
...
function alStatPixel() {
    ...
    (window.Image ? (new Image()) : document.createElement('img')).src = 'http://tl-stat.com/land/collect/?'+encodeQueryData(pixel_data);
}
...
function alGetData() {
    ...
    _scr.src = "http://tl-stat.com/land/params/?"+encodeQueryData(data);
    //_scr.async = true;
...
try{
    /*
     window.addEventListener('error', function (errorEvent) {
         ...
         (window.Image ? (new Image()) : document.createElement('img')).src = 'http://tl-stat.com/land/errors/?'+encodeQueryData(data);
     });
     */
