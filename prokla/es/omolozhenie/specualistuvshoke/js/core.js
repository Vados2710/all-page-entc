(function ($) {
  $(function () {
    setTimeout(function () {
      var stickTop = $('.good-to-know'),
        stickOffset = stickTop.offset();

      const stickBlock = document.querySelector(".good-to-know");
      ...
      $(window).scroll(function () {
        if ($(window).scrollTop() > stickOffset.top - 35) {
          ...
          if (stickBlock.getBoundingClientRect().bottom > footer.getBoundingClientRect().top - 20) {
            stickTop.css({
              position: 'fixed',
              top: -(stickBlock.getBoundingClientRect().bottom - footer.getBoundingClientRect().top)
            });
          }
        } else {
          stickTop.css({
...
function addMargin() {
  body.style.marginBottom = banner.offsetHeight + "px"
}
addMargin();
